...
# ПРАВИЛО: добавляешь колонку в SHEET_HEADERS одного из инкрементальных
# листов (events/workouts/subscriptions/coins/achievements — тех, что
# копятся через append_rows, а не переписываются целиком как users/
# equipment_items) — ensure_sheet_structure сам поправит ЗАГОЛОВОК на уже
# существующем листе, но СТРОКИ, записанные до этого изменения, он не
# трогает: append-only, воркер никогда не переписывает старые строки.
# У них будет на одну колонку меньше, и все данные правее точки вставки
# читаются под чужими подписями — не "не хватает данных", а вводящее в
# заблуждение смещение. Обязателен разовый бэкфилл (см. историю:
# scripts/backfill_sheets_username.py — регенерирует затронутые листы
# целиком из БД тем же кодом синка, безопаснее точечного патча старых
# строк на месте), не только правка заголовка.
