...
class ProgressPointResponse(BaseModel):
    ...
    (issue #97): вес легитимно скачет каждую вторую тренировку, что без
    пометки на графике читается как аномалия/сбой данных. Прокидывается из
    BlockAssignment.is_heavy напрямую, вне зависимости от metric — та же
    тренировка одинаково "тяжёлая" на всех трёх вкладках графика, не
    только на "Сила"."""
