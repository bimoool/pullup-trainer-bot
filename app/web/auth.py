...
#
# 3600 (issue #187) ломал реальные тренировки: initDataRaw читается ОДИН
# раз при открытии Mini App (webapp-frontend/src/App.tsx, useEffect on
# mount) и живёт в React-состоянии до закрытия — Telegram сам initData в
# открытом WebView не обновляет. Эта же проверка стоит на КАЖДОМ запросе
# (в т.ч. финальной отправке тренировки), не только на входе, поэтому
# сессия дольше часа (разминка + подходы + заминка — обычное дело)
# гарантированно упирается в 401 посреди тренировки. 12 часов — с запасом
# перекрывает любую разумную длину одного открытия Mini App, оставаясь
# несравнимо короче, чем время, за которое перехваченная initData вообще
# может быть кем-то использована повторно.
INIT_DATA_MAX_AGE_SECONDS = 12 * 3600
