...
async def handle_show_progress_report(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    # Недельная сводка скоупится по текущему (или только что закрытому)
    # циклу — та же логика, что и в app/workers/weekly_report.py (Часть 8
    # респека). current_equipment_progress намеренно остаётся на полной
    # истории — это динамика "на одном и том же снаряде", а не "за цикл".
...
@router.callback_query(F.data == "show_all_cycles_analytics")
async def handle_show_all_cycles_analytics(callback: CallbackQuery, session: AsyncSession) -> None:
    users = UserRepository(session)
    user = await users.get_by_telegram_id(callback.from_user.id)
    ...
    records = await workouts.list_records_for_user(user.id)
    if not records:
        await callback.message.answer(texts.ALL_CYCLES_EMPTY, reply_markup=progress_section_keyboard())
        await callback.answer()
        return

    analytics = all_cycles_analytics(records)
    await callback.message.answer(format_all_cycles_analytics(analytics), reply_markup=progress_section_keyboard())
    await callback.answer()
...
async def handle_export_xlsx(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    all_sets = await WorkoutSetRepository(session).list_for_user(user.id)
    set_numbers = {workout_set.id: workout_set.set_number for workout_set in all_sets}
