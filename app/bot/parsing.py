...
MIN_REPS = 0
# Разумный верхний предел на одно число — не тренировочная константа, просто
# защита от совсем абсурдных опечаток (лишний ноль и т.п.). НЕ путать с
# ANOMALY_LARGE_VALUE_THRESHOLD (50) — тот мягко переспрашивает, не
# блокирует; этот жёстко отклоняет ввод. Раньше был 100 — блокировал
# обычные трёхзначные результаты (например 102) как явный баг.
MAX_REPS = 999
