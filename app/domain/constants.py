...
# заниженный для того, кто уже у потолка (33). Заменён (issue #89) на
# множитель от текущей цели блока A (round(target * 1.5)), а затем (issue
# #105, живое использование) убран вообще — даже масштабируемый
# необязательный ориентир (цель 26 → подсказка 39) читался как обязательный
# резкий скачок, тот же класс проблемы, что был у фиксированных "50". Текст
# теста (VOLUME_DELOAD_PROMPT) больше не называет никакого числа.
