...
async def test_update_band_item_rejects_foreign_item(session):
    ...
    other = await UserRepository(session).create(telegram_id=53034, username="intruder")
...
async def test_delete_band_item_rejects_foreign_item(session):
    ...
    other = await UserRepository(session).create(telegram_id=53043, username="intruder2")
