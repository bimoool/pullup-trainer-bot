...
async def test_delete_band_item_already_used_in_workout_does_not_break_history(session):
    ...
    # user.id захвачен заранее (это уже обычный python int, не атрибут
    # ORM-объекта) — намеренно не через повторный
    # UserRepository.get_by_telegram_id ПОСЛЕ expire_all() ниже: тот вернул
    # бы тот же самый identity-map instance, что и user (совпадающий
    # telegram_id), и .id на нём в этот момент уже был бы expired-атрибутом.
    # Обращение к такому атрибуту синхронно (не под await) — ровно то же
    # исключение (MissingGreenlet), что чинит эта правка, только на другом
    # объекте: баг был не в WorkoutRepository, а в самом тесте.
    user_id = user.id
    ...
    target_a_state, target_b_state = await WorkoutRepository(session).resolve_next_targets(user_id)
